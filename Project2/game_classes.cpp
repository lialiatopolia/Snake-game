...
void Snake::apply_texture(SDL_Renderer* renderer, Game_Field* game_field) {
    // Задать цвет головы змейки
    SDL_Color head_color = { 255, 0, 0, SDL_ALPHA_OPAQUE }; // Красная голова
    SDL_Color body_color = { 0, 255, 0, SDL_ALPHA_OPAQUE }; // Зелёное тело

    // Рисуем голову змейки
    SDL_Rect head_rect = {
        i_x.at(0) * width + game_field->x,
        i_y.at(0) * height + game_field->y,
        width,
        height
    };
    ...
    // Добавляем глаза на голову змейки
    int eye_size = width / 4;
    SDL_Rect left_eye = {
        head_rect.x + width / 8,
        head_rect.y + height / 8,
        eye_size,
        eye_size
    };
    SDL_Rect right_eye = {
    ...
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, SDL_ALPHA_OPAQUE); // Черные глаза
    ...
    for (int i = 1; i < n; i++) {
        SDL_Rect body_rect = {
            i_x.at(i) * width + game_field->x,
            i_y.at(i) * height + game_field->y,
            width,
            height
        };

        // Плавный переход цвета от зелёного к желтому
        SDL_SetRenderDrawColor(renderer, body_color.r, body_color.g - (i * 10), body_color.b, body_color.a);
        SDL_RenderFillRect(renderer, &body_rect);

        // Добавляем закругленные углы ( прозрачными пикселями)
...
void Snake::eat(Food* food)
{
    if (i_x.at(0) == food->i_x && i_y.at(0) == food->i_y)
    {
        ...
        food->is_eaten = true;
            ...
            // Увеличиваем скорость на 25% после каждых 3-х съеденных блоков еды
            if (food_eaten % 3 == 0) {
                speed_factor *= 1.25f; // Увеличиваем множитель скорости
                std::cout << "Speed increased! Current factor: " << speed_factor << std::endl;
            }
        }
    }
...
void Snake::start(Game_Field* game_field)
{
    ...
    food_eaten = 0;       // Сбросить счётчик еды
    speed_factor = 1.0f;  // Начальная скорость (100%)
}
